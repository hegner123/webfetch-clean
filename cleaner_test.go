...
func TestCleanHTML_EmptyHTML(t *testing.T) {
	_, err := CleanHTML("", false, false, false, "clean")
	if err == nil {
...
func TestCleanHTML_PreserveMainOnly(t *testing.T) {
	...
	cleaned, err := CleanHTML(html, true, false, false, "clean")
	if err != nil {
		t.Fatalf("Expected no error, got: %v", err)
	}
...
func TestCleanHTML_RemoveImages(t *testing.T) {
	...
	cleaned, err := CleanHTML(html, false, true, false, "clean")
	if err != nil {
		t.Fatalf("Expected no error, got: %v", err)
	}
...
func TestCleanHTML_ScrapeMode_PreservesStructure(t *testing.T) {
	html := `
    <html>
        ...
        <body>
            ...
            <main>
                <h1>Title</h1>
                <p class="intro" data-section="1">Content</p>
            </main>
        </body>
    </html>
    `

	cleaned, err := CleanHTML(html, false, false, false, "scrape")
	if err != nil {
		t.Fatalf("Expected no error, got: %v", err)
	}

	// Scrape mode should preserve everything except <head>
	if strings.Contains(cleaned, "<head") {
		t.Error("Expected <head> to be removed in scrape mode")
	}
	if strings.Contains(cleaned, "<title") {
		t.Error("Expected <title> (inside head) to be removed in scrape mode")
	}

	// Scripts, styles, nav, footer, ads, iframes, sidebars, modals, cookies should all be preserved
	preserved := []string{
		"<script", "<style", "<nav", "Navigation",
		"Sidebar", "Ad content", "<iframe", "Footer content",
		"Cookies", "Modal",
	}
	for _, item := range preserved {
		if !strings.Contains(cleaned, item) {
			t.Errorf("Expected '%s' to be preserved in scrape mode", item)
		}
	}

	// Attributes should be preserved in scrape mode
	if !strings.Contains(cleaned, `class="intro"`) {
		t.Error("Expected class attribute to be preserved in scrape mode")
	}
	if !strings.Contains(cleaned, `data-section="1"`) {
		t.Error("Expected data attribute to be preserved in scrape mode")
	}
...
func TestCleanHTML_ScrapeMode_RemoveImages(t *testing.T) {
	html := `
    <html>
        <body>
            <img src="photo.jpg" alt="Photo">
	...
	cleaned, err := CleanHTML(html, false, true, false, "scrape")
	if err != nil {
		t.Fatalf("Expected no error, got: %v", err)
	}

	if strings.Contains(cleaned, "<img") {
		t.Error("Expected images to be removed even in scrape mode when removeImages=true")
	}
	if !strings.Contains(cleaned, "Text content") {
		t.Error("Expected text content to be preserved")
	}
...
func TestCleanHTML_ScrapeMode_StripLinks(t *testing.T) {
	html := `
    <html>
        <body>
	...
	cleaned, err := CleanHTML(html, false, false, true, "scrape")
	if err != nil {
		t.Fatalf("Expected no error, got: %v", err)
	}

	if strings.Contains(cleaned, "<a ") {
		t.Error("Expected links to be stripped even in scrape mode when stripLinks=true")
	}
	if !strings.Contains(cleaned, "Click here") {
		t.Error("Expected link text to be preserved")
	}
}

func TestCleanHTML_ScrapeMode_PreserveMainOnly(t *testing.T) {
	html := `
    <html>
        <body>
            <header>Header</header>
	...
	cleaned, err := CleanHTML(html, true, false, false, "scrape")
	if err != nil {
		t.Fatalf("Expected no error, got: %v", err)
	}
	...
	if strings.Contains(cleaned, "Header") {
		t.Error("Expected header to be removed when preserveMainOnly=true in scrape mode")
	}
}

func TestCleanHTML_DefaultModeIsClean(t *testing.T) {
	html := `
    <html>
        <body>
	...
	// Empty mode should default to clean
	...
	if strings.Contains(cleaned, "<script") {
		t.Error("Expected scripts to be removed with default (clean) mode")
	}
	if strings.Contains(cleaned, "<nav") {
		t.Error("Expected nav to be removed with default (clean) mode")
	}
	if !strings.Contains(cleaned, "Content") {
		t.Error("Expected content to be preserved")
	}
